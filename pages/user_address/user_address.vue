			...
			<view class="myaddress-lis" v-for="(item,index) in mainData" :key="index">
				...
				<view class="seltBox d-flex j-sb a-center">
					...
					<view class="R font-24 color9 d-flex a-center">
						<view class="child  d-flex a-center mr-4" :data-id="item.id" @click="Router.navigateTo({route:{path:'/pages/user_addressAdd/user_addressAdd?id='+$event.currentTarget.dataset.id}})"><image src="../../static/images/addressl-icon1.png" mode=""></image>编辑</view>
						<view class="child  d-flex a-center" :data-id="item.id" @click="deleteAddress($event.currentTarget.dataset.id)"><image src="../../static/images/addressl-icon2.png" mode=""></image>删除</view>
					</view>
				</view>
			</view>
